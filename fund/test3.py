...
class FundSpiders():
    ...
    def getFundInfo(self,fund_code):
        ...
        try:
            ...
            print (self.getCurrentTime(),'Fund Info Insert Sucess:', result['fund_code'],result['fund_name'],result['fund_abbr_name'],result['fund_type'],result['issue_date'],result['establish_date']
            ,result['establish_scale'],result['asset_value'],result['asset_value_date'], result['units'],result['units_date'],result['total_div']
            ,result['mgt_fee'],result['trust_fee'], result['sale_fee'], result['buy_fee'],result['buy_fee2'],result['benchmark'])
        except  Exception as e:
            print (self.getCurrentTime(), fund_code,fund_url,e )

        try:
            print (self.getCurrentTime(),'getFundInfo:', result['fund_code'],result['fund_name'],result['fund_abbr_name'],result['fund_type'],result['issue_date'],result['establish_date']
            ,result['establish_scale'],result['asset_value'],result['asset_value_date'], result['units'],result['units_date'],result['total_div']
            ,result['mgt_fee'],result['trust_fee'], result['sale_fee'], result['buy_fee'],result['buy_fee2'],result['benchmark']
            )
        except  Exception as e:
    ...
    def getFundNav(self,fund_code):
        ...
        '''
        try:
            #http://fund.eastmoney.com/bzdm.html#os_1;isall_1;ft_|;pt_1 基金实时净值
            #http://fund.eastmoney.com/bzdm.html#os_0;isall_1;ft_|;pt_1
            ...
            res = getURL(fund_url)
            #获取历史净值的总记录数
            startIdx = res.text.find("records:") + len("records:")
            endIdx = res.text.find(",pages:")
            records = res.text[startIdx:endIdx]
        except  Exception as e:
        ...
        for tr in tab.tbody.findAll('tr'):
            ...
            if tr.findAll('td') and len((tr.findAll('td')))==7 :
                ...
                try:
                    ...
                    # result['div']= tr.select('td:nth-of-type(7)')[0].getText().strip().strip('\'')
                ...
                try:
                    ...
                    print (self.getCurrentTime(),'fund_nav',str(i)+'/'+str(records),result['fund_code'],result['the_date'],result['nav'],result['add_nav'],result['nav_chg_rate'] )
                except  Exception as e:
            ...
            #如果是货币基金，获取万份收益和7日年化利率
                ...
                try:
                    ...
                    print (self.getCurrentTime(),'fund_nav_currency',str(i)+'/'+str(records),result['fund_code'], result['the_date'], result['profit_per_units'], result['profit_rate'] )
                except  Exception as e:
...
def main():
    ...
    mySQL._init_('localhost', 'root', 'wzj19960521', 'invest')
    header = randHeader()
    sleep_time = 0.1
    isproxy = 0
